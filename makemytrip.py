...
class MakeMyTrip(object):
    ...
    def print_json(self, l):
        ...
        for i in range(tmp_size):   
            ...
            print l[i]['le'][0]['an'] , "Rs.",l[i]['af'],"Duration: ", l[i]['td']
            result =result +str(l[i]['le'][0]['an']) +" "+ "Rs. "+str(l[i]['af']) +" "+"Duration: "+ str(l[i]['td'])+"\n"
            halt, layover=self.get_extra_detail(l[i]['le'])
            result = result + str(halt)+"\n"
            #ToDo
            print l[i]['le'][0]['fd'],  l[i]['le'][0]['fdt'], \
             u"  --->>  ", self.arrival_time
            result = result + str(l[i]['le'][0]['fd']) + " "+ str(l[i]['le'][0]['fdt'])+" "+str(\
            u"  --->>  ")+ " "+ str(self.arrival_time)+"\n"
            print "\tStoppage : ", layover
            print "Arrival : ", l[i]['le'][0]['fa'], l[i]['le'][0]['fat']
            # print u"\u2982"*50
            result = result + "\tStoppage : "+ layover+"\n\n"
            # result = result+"Arrival : "+str(l[i]['le'][0]['fa'])+" "+str(l[i]['le'][0]['fat'])+"\n\n"
    ...
    def send_email(self,message):
        sender = 'hackbot077@gmail.com'
        receiver = ['nanakksingh@gmail.com', 'ajaycl61@gmail.com']
        # message = """Subject : Hey this is a test mail from Python """
        msg = "\r\n".join([
        ...
        username = 'hackbot077@gmail.com'
        password = "hackbot@077"
        ...
        try:
            ...
            obj.sendmail(sender, receiver, msg)
            obj.quit()
    ...
    def send_sms(self,result):
        accountSID = 'ACe9ab8dd59a84ab40185f1e0afb190e71'
        authToken = 'becfa5b4486e1a420ef138b40d766480'
        twilioCli = TwilioRestClient(accountSID, authToken)
        myTwilioNumber = '+12564948953'
        myCellPhone = '+917259230033'
        message = twilioCli.messages.create(body=result, from_=myTwilioNumber, to=myCellPhone)
...
if __name__=="__main__":
    ...
    bro = MakeMyTrip()
    # def job():
    #     bro.print_json(bro.journey_oneway(origin,destination,my_date))
